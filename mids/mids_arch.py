...
class MIDS(nn.Module):
    def __init__(self, dim=768, **kwargs):
        super(MIDS, self).__init__()
    ...
    def get_cls_loss(self, logits, labels, ignore_index=-1):
        loss_func = nn.CrossEntropyLoss(ignore_index=ignore_index)
        loss = loss_func(logits, labels)
        return loss

    def forward(self, texts, image, labels=None, batch_size=1, N=1, M=1):
        # Feature encoding
        texts_features = self._encode_text(texts) # (batch*(1+N+M), seq_len, dim)
        ...
        if labels is not None:
            cls_loss = self.get_cls_loss(logits, labels)
        else:
            cls_loss = None
